...
(function () {
  ...
  function drawAll() {
    LRUI.updateDataChart(LRLOGIC.points, LRLOGIC.w, LRLOGIC.b);
  ...
  function syncMetrics() {
    ...
    if (els.manualW && document.activeElement !== els.manualW) {
      els.manualW.value = LRLOGIC.w.toFixed(4);
    }
  ...
  function syncManualParamsFromState() {
    if (els.manualW) els.manualW.value = LRLOGIC.w.toFixed(4);
    if (els.manualB) els.manualB.value = LRLOGIC.b.toFixed(4);
  }

  function applyManualModelParams() {
    const nextW = parseFloat(els.manualW.value, 10);
    const nextB = parseFloat(els.manualB.value, 10);
    const r = LRLOGIC.setModelParams(nextW, nextB);
    if (!r.ok) {
  ...
  dataChartEl.addEventListener("click", (ev) => {
    ...
    if (!LRLOGIC.tryAddPoint(p.x, p.y)) return;
    onDatasetChanged();
  });
  ...
  [els.manualW, els.manualB].forEach((el) => {
    if (!el) return;
