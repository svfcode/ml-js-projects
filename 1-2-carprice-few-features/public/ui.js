...
(function (global) {
    ...
    dataChart = new Chart(dataCanvasEl, {
      data: {
        datasets: [
          ...
          {
            type: "line",
            label: "ŷ при среднем объёме (сечение плоскости)",
            data: [],
  ...
  function meanEngineVolume(points) {
    const n = points.length;
    if (!n) return 2;
    let s = 0;
    for (const p of points) s += p.v;
    return s / n;
  }

  /** Линия на графике: фиксируем средний объём v̄, строим y = w₁·x + w₂·v̄ + b. */
  function updateDataChart(points, w1, w2, b) {
    if (!dataChart) return;
    dataChart.data.datasets[0].data = points.map((p) => ({
      x: p.x,
      y: p.y,
      v: p.v,
    }));
    ...
    const vBar = meanEngineVolume(points);
    dataChart.data.datasets[1].data = [
      { x: x0, y: w1 * x0 + w2 * vBar + b },
      { x: x1, y: w1 * x1 + w2 * vBar + b },
    ];
  ...
  function updateMetrics(els, w1, w2, b, loss, stepCount) {
  ...
  function applyModeChrome(els, state) {
    ...
    if (els.manualEngineRow) els.manualEngineRow.hidden = !manual;
